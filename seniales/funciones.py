...
def graficar_seniales(seniales, t, nombres, random_colors = True, window_lenght = 10, window_height = 2):
    ...
    Args:
        seniales (array o list): Una señal o una lista de señales [s1, s2, ...] a graficar.
        t (array): Es la dimensión temporal base, se le asigna a cada señal una nueva dimensión temporal en base a su cambio.
        nombres (str o list): Un nombre o una lista de nombres ["n1", "n2", ...] de las señales.
    ...
    if not isinstance(seniales, list):
        seniales_arreglo = [seniales]
    if not isinstance(nombres, list):
        nombres_arreglo = [nombres]
    
    N = len(seniales_arreglo)
    fig, axs = plt.subplots(N, 1, figsize=(window_lenght, window_height * N))
    
    # Si hay solo una señal, axs es un objeto Axes, no un array
    if N == 1:
        axs = [axs]
    ...
    for i in range(N):
        if (t[1] - t[0] == 1):
            ...
            dim_t = np.arange(t[0], t[0] + len(seniales_arreglo[i]), 1)  # Se genera una dimensión temporal en base al tamaño de la señal
            if random_colors:
                axs[i].stem(dim_t, seniales_arreglo[i])
            else:
                axs[i].stem(dim_t, seniales_arreglo[i])
            axs[i].set_title(nombres_arreglo[i])
            axs[i].grid()
        else:
            # Si la dimensión temporal es menor a 1, se asume que es una señal discreta y se grafica con marcadores.
            dim_t = dominio_temporal(t, seniales_arreglo[i])
            if random_colors:
                axs[i].plot(dim_t, seniales_arreglo[i], color=colors[random.randint(0, len(colors)-1)])
            else:
                axs[i].plot(dim_t, seniales_arreglo[i], color=colors[i % len(colors)])
            axs[i].set_title(nombres_arreglo[i])
            axs[i].grid()
